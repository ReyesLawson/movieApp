...
function GenreWeb() {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=${apiKey}`)
      ...
      .then(data => {
        console.log('Fetched Genres:', data.genres);  // Debugging
        setGenres(data.genres || []);  // Ensure genres is always an array
      })
      .catch(error => console.error('Error fetching genres:', error));  // Handle fetch errors
  }, []);

  const fetchMovies = () => {
    if (!selectedGenre) {
      console.warn('No genre selected');  // Debugging
      return;  // Prevent fetching if no genre is selected
    }
    ...
    fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_genres=${selectedGenre}`)
      .then(response => response.json())
      .then(data => {
        console.log('Fetched Movies:', data.results);  // Debugging
        setMovies(data.results || []);  // Ensure movies is always an array
      })
      .catch(error => console.error('Error fetching movies:', error));  // Handle fetch errors
  };

  const getRandomMovie = () => {
    console.log('Movies array:', movies);  // Debugging
    if (!movies || movies.length === 0) {
      console.warn('No movies available to suggest');  // Debugging
